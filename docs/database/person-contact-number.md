...
| `INTERNET_PHONE` | 허용 | 전역 유일 제약 없음 |
| `PERSONAL_NUMBER_SERVICE` | 허용 | 전역 유일 제약 없음 |

MOBILE은 현재 업무에서 개인 휴대전화로 취급한다. 나머지 세 종류는 여러 Person이 공유할 수 있는 업무 연락처로 취급한다. 이는 현실의 모든 사용 사례를 일반화한 규칙이 아니며 실제 업무에서 문제가 확인되면 재검토한다.
